...
import { Search, Pin, Plus, MoreVertical, LayoutGrid, List, Clock, Filter, CheckCircle2, XCircle, Loader2, Trash2, Save } from 'lucide-react';
        ...
        <div className="bg-background-light dark:bg-background-dark min-h-screen pb-32">
            ...
            <header className="px-6 py-4 space-y-4 sticky top-0 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md z-40 max-w-7xl mx-auto w-full">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-black tracking-tight">Family Notes</h1>
            ...
            <main className="max-w-7xl mx-auto px-6 space-y-8">
                ...
                {pinnedNotes.length > 0 && (
                    <section className="space-y-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Pinned Notes</h2>
                        </div>
                ...
                <section className="space-y-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Recent Notes</h2>
                        <div className="flex bg-white dark:bg-slate-800 rounded-lg p-1 shadow-sm">
                            <button className="p-1.5 text-primary bg-primary/10 rounded-lg"><LayoutGrid size={14} /></button>
                            <button className="p-1.5 text-slate-400"><List size={14} /></button>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl overflow-hidden flex flex-col max-h-[90vh] shadow-2xl">
                        <header className="px-6 py-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
                            <button onClick={() => setIsEditing(null)} className="text-slate-400 hover:text-slate-600 transition-colors"><XCircle /></button>
                            <div className="flex gap-2">
                                {isEditing.id && (
                                    <button onClick={() => handleDelete(isEditing.id)} className="p-2 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors"><Trash2 size={20} /></button>
                                )}
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-4">
                            ...
                            <textarea
                                ...
                                placeholder="Write your family note here..."
...
const NoteCard: React.FC<{ note: Note, onTogglePin: (id: string, s: boolean) => void, onClick: () => void }> = ({ note, onTogglePin, onClick }) => (
    ...
    >
        <div className="flex justify-between items-start mb-2">
            <h3 className="font-bold text-sm line-clamp-2 leading-tight pr-4">{note.title || 'Untitled'}</h3>
            <button
            ...
            >
                <Pin size={14} className={note.is_pinned ? 'fill-primary' : ''} />
            </button>
        </div>
